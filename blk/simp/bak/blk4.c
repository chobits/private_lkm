...
static int fool_make_request(struct request_queue *q, struct bio *bio)
{
	...
	for (i = bio->bi_idx, bv = &(bio->bi_io_vec[i]);
				i < bio->bi_vcnt; i++, bv++) {
		disk_mem = get_disk_addr(bio->bi_sector << 9);
		offset = (bio->bi_sector << 9) % PAGE_SIZE;
		len = (bv->bv_len > PAGE_SIZE - offset) ?
			(PAGE_SIZE - offset) : (bv->bv_len);
		...
		case WRITE:
			...
			if (bv->bv_len > len) {
				printk(KERN_INFO MD_NAME":write off %d\n", bv->bv_len - len);
				disk_mem = get_disk_addr((bio->bi_sector << 9) + PAGE_SIZE);
				memcpy(disk_mem, iovec_mem + len, bv->bv_len - len);
		...
		case READA:
			...
			if (bv->bv_len > len) {
				printk(KERN_INFO MD_NAME":read off %d\n", bv->bv_len - len);
				disk_mem = get_disk_addr((bio->bi_sector << 9) + PAGE_SIZE);
				memcpy(iovec_mem + len, disk_mem, bv->bv_len - len);
			}
			break;
