 ...
 * sub traverses:
 *  + using radix_tree_gang_lookup instead of radix_tree_lookup
 *  + using page->index as radix index
 */
...
static void free_diskmem(unsigned int size) 
{
	...
	while (segs) {
		...
		n = radix_tree_gang_lookup(root, (void **)pages, seg, 64);
		for (k = 0; k < n; k++) {
			page = pages[k];
			if (!page) {
				/* May it be here? */
				printk(KERN_ALERT "radix tree err\n");				
			}

			seg = page->index;
			__free_pages(page, SEGMENT_ORDER);
			radix_tree_delete(root, seg);
		}
		segs--;
	}
}
...
static struct radix_tree_root *alloc_diskmem(unsigned int size)
{
	...
	for (i = 0; i < segs; i++) {
		...
		/* page->index is not used here, so we use it trickly */
		page->index = i;
		/* debug_page(page); */
	}
	return root;
