...
#include <linux/tty.h>			/* MAX_NR_CONSOLES */
 ...
 *
 * vt_ioctl->setledstat->set_leds->tasklet_schedule(&keyboard_tasklet)
 *
...
int kbleds_init(void)
{
	...
	for (i = 0; i < MAX_NR_CONSOLES; i++) {
		if (!vc_cons[i].d)
			break;
		printk(KERN_ALERT "poet_atkm: console[%i/%i] #%i, tty %lx\n", i,
		       MAX_NR_CONSOLES, vc_cons[i].d->vc_num,
		       (unsigned long)vc_cons[i].d->vc_tty);
	}
	printk(KERN_ALERT "kbleds: finished scanning consoles\n");
...
{
	printk(KERN_ALERT "kbleds: unloading...\n");
	del_timer(&my_timer);
	(my_driver->ops->ioctl) (vc_cons[fg_console].d->vc_tty, NULL, KDSETLED,
			    RESTORE_LEDS);
}
